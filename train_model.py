...
numeric = ["Optimal_N", "Optimal_P", "Optimal_K", "Optimal_pH", "Expected_Yield_ha", "Optimal_Rainfall"]  
for col in numeric:
...
for _ in range(5000):
    ...
    on, op, ok, oph, base, o_rain = float(r["Optimal_N"]), float(r["Optimal_P"]), float(r["Optimal_K"]), float(r["Optimal_pH"]), float(r["Expected_Yield_ha"]), float(r["Optimal_Rainfall"])  # ✅ добавь o_rain
    ...
    rain = max(100, o_rain + np.random.uniform(-o_rain*0.3, o_rain*0.3))  
    ...
    rain_min = o_rain * 0.65
    rain_max = o_rain * 1.35
    if rain < rain_min or rain > rain_max:
        if rain < rain_min:
            penalty = 0.2 * (1 - rain / rain_min)
        else:
            penalty = 0.2 * (rain / rain_max - 1)
        factor *= (1 - penalty)

    factor = max(0.3, factor)
    noise = np.random.normal(0, 0.05 * base)
    y = max(0, base * factor + noise)
